...
driven round: `score --run <dir>` re-graded the artifacts already on disk and
filled `gates_held`, and `report --redraw` rewrote the table without touching a
history row. Two cells moved.

Cursor's T1-deck reads **`pass (14 held)`** — fourteen of the eighteen gating
rows had something to grade in it. The four that did not are the two imagery
gates on a deck with no image, `D21_data_contract` on a deck where no figure
declares its data, and `D32_shape_use` on a deck with no page declaring an
analysis move. It is the only deck on the board with a count, because it is the
only T1 that earned a verdict at all: Claude Code ran past the hour cap, Hermes
wrote its file outside the working directory, Gemini was rate-limited.

And Gemini's row went from `not run` to **`run, nothing earned: 3 of 3
attempted`** — 0.1.609 wrote that distinction and nothing on the board could
show it until the cells carried the `attempted` field. An agent run three times
and rate-limited three times is not an agent nobody drove.

**Re-scoring with today's instruments changed no verdict**, which is worth
recording because it was not guaranteed: 0.1.609 widened D6's vocabulary and
0.1.610 added the held count, and the 0.1.610 checkers answered these August
artifacts exactly as the 0.1.605 checkers had. Twelve cells, zero drift.

**What it did break is a pin, silently, and finding that is the release.** Every
history row carries `scores_sha256`, and the code that writes it says why: the
run directory lives outside this repository and is gitignored, so **the digest
is the whole of what makes a row evidence rather than an assertion**. Re-scoring
rewrites `scores.json`; all four rows for that run stopped resolving to it, in a
command whose output was otherwise nothing but success. The verdicts being
identical is exactly the case where such a break goes unnoticed.

`score` now says so, and **keeps the bytes rather than recommending that
somebody should have**. It reads before the write, so the note can copy the file
it is about to lose; the previous `scores.json` lands beside the new one named
for the instruments that produced it. Reported, never a gate — re-scoring with
newer instruments is legitimate, and refusing it would be worse than naming what
it costs — and the note warns that `report --record` is not the remedy, because
it would stamp fresh rows with today's skill version over agents that read an
older one.

**The first version of that check was the seventh FM-01 in this repository, and
a review found it before the merge.** `scores.json` is rewritten near the top of
the branch, and the `if not scores` early return leaves before the note — so
the one case where the pin is destroyed COMPLETELY rather than partially, the
file replaced by `{}`, was the one case the check could never reach. It is
reachable by pointing `--run` one directory too high, or at a results tree
somebody tidied. The check now runs before the write and prints on both exits.

Two more of the same shape, in the same function. `json.JSONDecodeError`
subclasses `ValueError`, so one `except` around the parse turned "this history
file is a merge conflict" into "nothing is wrong" — on a tracked file two
branches both append to, which makes it the likeliest thing here to arrive
unparseable, and the only evidence store there is. And `null`, a number and an
object all parse as valid JSON, so three shapes reached a list comprehension
that raised `TypeError` **after** the scores file had been replaced: a stack
trace, a destroyed pin and no note. The three answers are now distinct —
"nothing to break", "this command will break these", and "I could not look, so
the effect is UNKNOWN, not clean".

**And it claimed a replacement it had not made.** Comparing the current file
against the recorded digests meant every later invocation printed the same
sentence, flattening the one signal that tells an operator whether their own
command is the cause. Reading before the write separates "about to replace"
from "already pinned a different one".

Smaller, from the same review: `expanduser()` raises `RuntimeError` on
`~nosuchuser` and `resolve()` raises `ValueError` on an embedded NUL, neither
caught by the `except OSError` written for "a path this machine cannot
resolve"; a relative history row (`conformance/results/latest`, and eighteen of
the thirty-six rows are relative) resolved against the shell's directory rather
than the repository's, so running `score` from anywhere else turned real rows
into phantoms; and the board's own first line read `generated by
run_conformance.py report --record` whatever wrote it, so a board written by
`--redraw` was stamped as recorded — in the release whose argument is that the
two are different. The marker names the command now and the previous form is
still accepted when reading, so no board has to be regenerated to stay legible.

One finding from that review was **withdrawn after checking**: `score --run A
--run B` does not silently score only A. It refuses, at a guard written for
exactly that, and exits 1.

One more silent miss, found by asking the function the question it exists to
ask. Its comparison ran on `_portable`, which collapses `$HOME` to `~` and
nothing else — so a run directory recorded as `conformance/results/latest` and
given as `./conformance/results/latest` read as two different directories and
the warning quietly did not fire. Both sides resolve now, with the portable
string as the fallback for a path this machine cannot resolve.

Deliberate red, seven runs: run the guard after the write, drop the note from
the empty-scores exit, let an unreadable history go quiet, delete the
not-a-list check, stop keeping the bytes, compare the post-write file, and
compare the paths as strings again. **Two did not plant on the first
attempt.** The path fixture built its awkward path with `pathlib`, which collapses `.` while
constructing, so the two spellings were the same string and the test passed
against the very defect it names. And every test in the file called the check
directly, so moving it back after the write — the original defect, exactly —
left all thirteen green. Convention 15 says run the planted failure first; this
is the case for running it and then LOOKING, because a red that does not go red
is indistinguishable from a fix that works. The first version of the test file called `_pin_broken`
directly and nothing else, so deleting the two lines that call it from `score`
left every test green — the warning existed and reached nobody. There is an
end-to-end test now, and deleting the call site fails it.
