...
- **`D14`** Let no author placeholder reach the reader. — `references/design-rules.md:1155`
- **`D14`** Ship no [TO FILL] placeholder in a finished document. — `references/storyline-templates.md:566`
- **`D15`** Never print a repository or file path in a footer source line. — `references/design-rules.md:1147`
...
- **`D22`** Give every page a layout class the shipped tokens define. — `references/design-rules.md:1521`
- **`D24`** Ship every image inside the file as a data: URI, never as a link. — `SKILL.md:437`
- **`D24`** Embed every image in the file as a data: URI rather than linking it. — `references/design-rules.md:1635`
- **`D25`** Name every image's source and licence terms in the document. — `SKILL.md:438`
- **`D25`** Name every image's origin and terms on the page. — `references/design-rules.md:1639`
- **`D25`** Name the licence for any image that is not public domain or CC0. — `references/design-rules.md:1642`
- **`D33`** Take every icon from the two sets this package ships; never draw one ad hoc. — `references/design-rules.md:1206`
...
- **`collision`** Let nothing land on anything: no text on text and no text on any drawn element. — `references/design-rules.md:1474`
- **`content_hidden`** Never clamp or hide overflow in a title block. — `references/design-rules.md:1501`
...
- **`reserve_overspent`** Treat the title block's reserved height as a ceiling. — `references/design-rules.md:1492`
- **`role_split`** Render every repeating role exactly one way across the deck. — `references/brand.md:235`
- **`role_split`** Render each repeated role exactly one way across the document. — `references/design-rules.md:1436`
- **`title_two_lines`** Keep a title to one line where it fits and never past two. — `references/design-rules.md:1243`
...
- **`opener_subject_mark`** Use the filled silhouette set only for part-opener subject marks. — `references/design-rules.md:1174`
- **`opener_subject_mark`** Make an opener's subject mark fill-based, never stroke-based. — `references/design-rules.md:1233`
...
- **`D20`** Retint a library shape only along the accent ladder, never by introducing a colour. — `references/design-rules.md:1104`
...
- **`figure_clipped`** Keep every drawn element inside its figure's viewBox. — `references/design-rules.md:1411`
...
- **`figure_viewbox`** Edit a figure's viewBox with the shape, so the box a figure declares is the box it draws in. — `references/design-rules.md:1412`
...
- **`starved_column`** Start side-by-side cells on one line with comparable weight. — `references/design-rules.md:1563`
